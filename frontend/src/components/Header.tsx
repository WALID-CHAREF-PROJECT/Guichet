...
export default function Header(): JSX.Element {
  ...
  useEffect(() => {
    const sync = (): void => setUser(getCurrentUser());
      ...
      <div className="mx-auto max-w-[1700px] space-y-4 px-4 py-4 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden items-center gap-2 md:flex">
            ...
            <Link to="/ma-fr/panier" className="relative rounded-full bg-white/10 px-3 py-1 text-sm">🛒
              <span className="absolute -right-2 -top-2 rounded-full bg-orange-500 px-1.5 text-[10px] font-semibold text-white">{totals.totalQuantity}</span>
            </Link>
            ...
            {user ? (
              <>
                ...
                <button onClick={() => { setCurrentUser(null); setUser(null); }} className="rounded bg-white/10 px-3 py-1 text-xs">{translate('logout')}</button>
              </>
            ) : (
