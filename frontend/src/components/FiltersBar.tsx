...
const fieldClass = 'rounded-md border border-white/20 bg-[#082252] px-3 py-2 text-sm';

export default function FiltersBar(props: Props): JSX.Element {
  return (
    <section className="grid gap-3 rounded-2xl border border-white/10 bg-white/5 p-4 md:grid-cols-5">
      <input value={props.search} onChange={(e) => props.onChange('search', e.target.value)} placeholder="Rechercher un événement" className={fieldClass} />
      <select value={props.category} onChange={(e) => props.onChange('category', e.target.value)} className={fieldClass}>
        <option value="">Toutes catégories</option>
        {props.categories.map((item) => <option key={item.id} value={item.slug}>{item.name}</option>)}
      </select>
      <select value={props.city} onChange={(e) => props.onChange('city', e.target.value)} className={fieldClass}>
