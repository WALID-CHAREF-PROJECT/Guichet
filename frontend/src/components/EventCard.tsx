...
export default function EventCard({ event }: Props): JSX.Element {
  ...
  return (
    <article
      ...
      className="group relative cursor-pointer overflow-hidden rounded-2xl border border-sky-100/15 bg-gradient-to-b from-[#0c2247]/95 to-[#06152f]/95 shadow-glass transition duration-300 hover:-translate-y-1 hover:border-sky-200/40 hover:shadow-[0_18px_42px_rgba(8,47,110,0.45)] focus:outline-none focus:ring-2 focus:ring-sky-300"
    >
      ...
      {event.badge && <span className="absolute left-3 top-3 rounded-full border border-white/15 bg-slate-950/65 px-3 py-1 text-xs text-white backdrop-blur-md">{event.badge}</span>}
      {isPlanEvent && <span className="absolute left-3 top-3 rounded-full bg-amber-500/95 px-3 py-1 text-xs font-semibold text-[#241300] shadow-lg shadow-amber-500/30 backdrop-blur">Plan interactif</span>}
      <div className="space-y-2 p-4">
        ...
        <div className="flex items-center justify-between gap-2 pt-1">
          <span className="font-semibold text-amber-300">{event.is_free ? 'Gratuit' : `${event.price_mad} MAD`}</span>
          <div className="flex items-center gap-2">
            <button type="button" onClick={handleAddToCart} className="rounded-full border border-amber-200/40 bg-gradient-to-r from-amber-400 to-orange-400 px-3 py-1 text-sm font-semibold text-[#2d1a00] shadow-md shadow-amber-500/20 transition hover:brightness-105">{isPlanEvent ? 'Voir le plan' : 'Acheter'}</button>
            <span className="rounded-full border border-sky-100/35 bg-white/[0.02] px-3 py-1 text-sm font-medium text-slate-100 transition group-hover:border-sky-200/60 group-hover:bg-white group-hover:text-[#041743]">Voir</span>
          </div>
        </div>
      </div>
    </article>
