...
export function AdminProducerCreatePage(): JSX.Element {
  ...
  return <section className="space-y-6 rounded-[2rem] border border-white/10 bg-gradient-to-br from-slate-950 via-[#061a4d] to-slate-950 p-6 text-white shadow-2xl"><div><p className="text-sm uppercase tracking-[0.28em] text-cyan-200/70">Nouveau fournisseur</p><h1 className="text-3xl font-black">Créer un fournisseur complet</h1></div>{error && <p className="rounded-2xl bg-red-500/15 p-3 text-red-100">{error}</p>}
    <form className="grid gap-5" onSubmit={(e) => void submit(e)}><div className={card + ' grid gap-4 md:grid-cols-2'}><input className={input} placeholder="Prénom" value={form.firstName} onChange={(e) => set('firstName', e.target.value)} required /><input className={input} placeholder="Nom" value={form.lastName} onChange={(e) => set('lastName', e.target.value)} required /><input className={input} type="email" placeholder="Email" value={form.email} onChange={(e) => set('email', e.target.value)} required /><input className={input} type="password" placeholder="Mot de passe" value={form.password} onChange={(e) => set('password', e.target.value)} required /><input className={input} placeholder="Téléphone" value={form.phone} onChange={(e) => set('phone', e.target.value)} /><input className={input} placeholder="Entreprise" value={form.name} onChange={(e) => { set('name', e.target.value); if (!form.slug) set('slug', asciiSlug(e.target.value)); }} required /><input className={input} placeholder="Slug" value={form.slug} onChange={(e) => set('slug', asciiSlug(e.target.value))} required /><select className={input} value={form.is_active ? '1' : '0'} onChange={(e) => set('is_active', e.target.value === '1')}><option value="1">Actif</option><option value="0">Inactif</option></select></div>
